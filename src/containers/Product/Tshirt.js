...
const Tshirt =(props) => {
	...
	const [{tshirts, filteredPagesT, currentPageT, filteredTshirts}, dispatch ] = useStateValue();
   ...
   const previousPage =() => {
	...
	const onClickFilter = () => {
		...
		dispatch({
				type: 'FILTER_BY_PRICE_SHIRT',
				price: {"min":min.value.start,"max":min.value.end}
			});
	}
			  
			  
		const result = tshirts.filter(item => item.top == 1)
				...
				<Col xs={12} md={8} className="m-2">
						<Row>
							...
							<div className="d-flex">
								...
								<div className="mr-auto">Showing-{currentPageT}/{filteredPagesT}</div>
					...
					<Col className='mt-3'>
						<section className='section'>
                            ...
                            <ul className="pagination-list">
                                {
                                    [...Array(filteredPagesT)].map((value, index) => (
                                        <Button
										variant="secondary" 
										className="mr-1"
                                            aria-label="Page 1"
